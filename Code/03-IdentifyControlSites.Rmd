---
title: "Identify host sites for nonhost sites based on climate and tree-ring data"
...
2.  the nonhost chronology was less than 150 km away from the host site.
...
for(j in res$host){
  ...
  nonhost.long <- nonhost.meta %>% filter(LastYear>=lastyrj) %>% pull(SeriesCode)
  nonhost.long <- which(nonhost.long %in% colnames(dists))
  
  # (2) limit to sites within 150 km 
  ...
  nada.ts <- ts(nada.hosts[,j], start=1650)
...
res <- res %>% select(host, Control1, Control2, Control3, r.hostxNADA, notes)

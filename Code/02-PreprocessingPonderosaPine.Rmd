---
title: "Preprocessing of ponderosa pine chronologies"
  ...
  tryCatch(rwlj  <- read.rwl(filej), error = function(e) { skip = TRUE})
   if(skip==TRUE) {next} else{
    ...
    rwl.summaryj <- summary.rwl(rwlj)
     ...
     keymet[keymet$`SeriesCode`==series, "FirstYear"] <- rwl.reportj$firstYear# calculate the number of series
     keymet[keymet$`SeriesCode`==series, "LastYear"] <- rwl.reportj$lastYear# calculate the number of series
...
We computed the correlation between each tree-ring series using the *corr.rwl.seg()* function in the dplR package. 
